...
def down(nyaa_list):
    ...
    try:
        ...
        html_url = []
        # 防止插入相同内容URL
        for i in re.findall(https_pattern, torrent_text.html):
            add_unique_element(html_url, i)
        #print(html_url)
    #    if len(tag_img)>0:
    #        for i in tag_img[0]:
    #            src = i.attr('src')
...
#
#
    #            elif re.search('^http[s]?://[\w\W]{0,2}hentai\.org/.*$', str_b):
    #                page.download()
    #                downimg(str_b,nyaa_list)
    #            elif re.search('^http[s]?://[\w\W]{0,2}\.wp\.com/.*$', str_b):
    #                page.download()
    #            elif re.search('^http[s]?://i.imgur.com/.*.[jpg|bmp|png|jpeg|webp|gif]$', str_b):
    #                page.download()
    #            # 不在抓取范围,结束抓取并记录
    finally:
